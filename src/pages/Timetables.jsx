...
const Timetables = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Assuming requiredData is the timetable ID – adjust if different
        const requiredData = useAuthStore.getState().requiredData;
        if (requiredData) {
          await getTable(requiredData);
        } else {
          console.warn("No requiredData (timetable ID) available");
        }
      } catch (err) {
    ...
    if (!gottenTable && !isLoading && !error) {
      fetchData();
    }
  }, [gottenTable, isLoading, error, getTable]);
  ...
  if (authLoading) {
    return (
      ...
      >
        <div className="timetables-loading-spinner" />
      </div>
    );
  }
  ...
  if (isLoading) {
    return (
      <>
        ...
        />
        <div
          className="p-8 max-w-md mx-auto space-y-6 animate-pulse"
        ...
        >
            ...
            <h3 className="text-xl font-medium text-gray-700 flex justify-center items-center">
              ...
              <span className="ml-2 flex space-x-1">
                {[...Array(3)].map((_, i) => (
                  <span
                    key={i}
                    className="h-1.5 w-1.5 bg-blue-500 rounded-full inline-block"
                    style={{
          ...
          <div className="flex justify-center">
            <div className="relative w-24 h-32">
              <div
                className="absolute inset-0 bg-blue-100 rounded-lg shadow-md transform origin-left"
                style={{animation: "flip 1.5s ease-in-out infinite"}}
              />
              <div
                className="absolute inset-0 bg-blue-50 rounded-lg shadow-md transform origin-right"
                style={{animation: "flip 1.5s ease-in-out infinite reverse"}}
              />
          ...
          <div className="grid grid-cols-7 gap-1 text-center">
            ...
            {[...Array(35)].map((_, i) => (
              <div
                key={i}
                className={`h-6 rounded-sm ${i % 8 === 0 ? "bg-blue-200" : "bg-gray-100"} animate-pulse`}
        ...
        <thead>
          <tr>
            ...
            {selectedTimetable.schedule?.map((day) => (
              <th
                key={day.day}
                className="border p-2 bg-gray-100 min-w-[150px]"
              >
                {day.day}
              </th>
            ))}
            ...
            return (
              <tr key={`period-${periodIdx}`}>
                ...
                {selectedTimetable.schedule.map((day) => {
                  ...
                  if (!currentPeriod)
                    return (
                      <td key={`${day.day}-empty`} className="border p-2"></td>
                    );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {doublePeriods.map((dp, index) => (
            ...
            >
              ...
              <div className="text-sm">
                <span className="text-gray-600">Duration:</span> {dp.duration}{" "}
                mins
              </div>
              <div className="text-sm">
  ...
  return (
    <>
      ...
      />
      <div
        className="relative min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 overflow-auto"
        style={{paddingTop: "68px"}}
      >
